...
def union_of_intervals(intervals: List[Interval]) -> List[Interval]:
  ...
  for i in intervals[1:]:
    if (i.left.val < result[-1].right.val
        or (i.left.val == result[-1].right.val) and
        (i.left.is_closed or result[-1].right.is_closed)):
      if (i.right.val > result[-1].right.val
          or (i.right.val == result[-1].right.val and i.right.is_closed)):
...
if __name__ == '__main__':
  exit(
    generic_test.generic_test_main('intervals_union.py', 'intervals_union.tsv',
